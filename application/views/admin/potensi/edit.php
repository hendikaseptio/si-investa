...
<section>
	<div class="container fluid">
		<h3>Edit Data Potensi Investasi</h3>
		<div class="card shadow">
			<div class="card-header bg-primary">Form Edit potensi</div>
			<div class="card-body">
				<form action="<?= site_url('admin/potensi/update/'.$data['id'])?>" method="post" enctype="multipart/form-data">
					<!-- <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>" /> -->
					<div class="mb-3">
						<label for="nama">Nama Proyek <small class="sup text-danger">*</small></label>
						<input type="text" name="nama" id="nama" class="form-control" placeholder="Masukkan Nama Proyek" value="<?= $data["nama"]?>" autofocus required>
					</div>
					<div class="mb-3">
					...
					<div class="mb-3">
						<div class="row">
							<div class="col-6">
								...
								<input type="text" name="lat" id="lat" class="form-control" placeholder="Misalnya 8.347435" value="<?= $data["latitude"]?>" required>
							</div>
							<div class="col-6">
								<label for="long">Logtitude <small class="sup text-danger">*</small></label>
								<input type="text" name="long" id="long" class="form-control" placeholder="Misalnya -11.34345890" value="<?= $data["longtitude"]?>" required>
							</div>
						</div>
					</div>
					<div class="mb-3">
					...
					<div class="mb-3">
						...
						<textarea name="lokasi" id="lokasi" cols="30" rows="3" class="form-control" placeholder="Masukkan alamat lengkap seperti jl,rt/rw,kel.,kec.,kota" required><?= $data["lokasi"] ?></textarea>
					</div>
					<div class="mb-3">
						<label for="detail_proyek">Detail Proyek <small class="sup text-danger">*</small></label>
						<textarea name="detail_proyek" id="detail_proyek" cols="30" rows="3" class="form-control" placeholder="Jelaskan tentang proyek tersebut" required><?= $data["detail_proyek"] ?></textarea>
					</div>
					<div class="mb-3">
						<label for="kondisi_eksisting">Kondisi Eksisting <small class="sup text-danger">*</small></label>
						<textarea name="kondisi_eksisting" id="kondisi_eksisting" cols="30" rows="3" class="form-control" placeholder="Jelaskan Kondisi Saat Ini" required><?= $data["kondisi_eksisting"] ?></textarea>
					</div>
					<div class="mb-3">
						<label for="peluang_investasi">Peluang Investasi <small class="sup text-danger">*</small></label>
						<textarea name="peluang_investasi" id="peluang_investasi" cols="30" rows="3" class="form-control" placeholder="Jelaskan keuntungan investor jika berinvestasi pada proyek ini" required><?= $data["peluang_investasi"] ?></textarea>
					</div>
					<div class="mb-3">
					...
					<div class="row">
						<div class="col-md-6">
							<div class="row">
								<div class="col-6">
									<?php $foto = explode(",", $data['foto']); ?>
									<div class="mb-3">
										<label for="foto1">Foto 1</label>
										<input type="file" name="foto1" id="foto1" class="form-control" accept=".jpg, .png, image/jpeg, image/png">
										<img src="<?= (!empty($foto[0]))? site_url('assets/foto/'.$foto[0]):"#" ?>" id="prev1" class="img-fluid shadow mt-2">
									</div>
									<div class="mb-3">
										<label for="foto2">Foto 2</label>
										<input type="file" name="foto2" id="foto2" class="form-control" accept=".jpg, .png, image/jpeg, image/png">
										<img src="<?= (!empty($foto[1]))? site_url('assets/foto/'.$foto[1]):"#" ?>" id="prev2" class="img-fluid shadow mt-2">
									</div>		
								</div>
								<div class="col-6">
									<div class="mb-3">
										...
										<img src="<?= (!empty($foto[2]))? site_url('assets/foto/'.$foto[2]):"#" ?>" id="prev3" class="img-fluid shadow mt-2">
									</div>
									<div class="mb-3">
										<label for="foto4">Foto 4</label>
										<input type="file" name="foto4" id="foto4" class="form-control" accept=".jpg, .png, image/jpeg, image/png">
										<img src="<?= (!empty($foto[3]))? site_url('assets/foto/'.$foto[3]):"#" ?>" id="prev4" class="img-fluid shadow mt-2">
									</div>
								</div>
							</div>
						</div>
