...
<section>
	<div class="container fluid">
		...
		<div class="card shadow">
			<div class="card-header bg-primary">Form Edit profil</div>
			<div class="card-body">
				<form action="<?= site_url('admin/profil/update/'.$data['id'])?>" method="post" enctype="multipart/form-data">
					<!-- <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>" /> -->
					<div class="mb-3">
						<label for="judul">Judul Profil<small class="sup text-danger">*</small></label>
						<input type="text" name="judul" id="judul" class="form-control" placeholder="Masukkan Judul Profil" value="<?= $data['judul'] ?>" autofocus required>
					</div>
					<div class="mb-3">
						<label for="deskripsi">Detail <small class="sup text-danger">*</small></label>
						<textarea name="deskripsi" id="deskripsi" cols="30" rows="3" class="form-control" placeholder="Jelaskan tentang proyek tersebut" required><?= $data['deskripsi'] ?></textarea>
					</div>
					<div class="mb-3">
